...
##      Jazz18Heap.py version xy15 022120
...
##     MatchTable = <Set True to obtain a 'Ref-By-Ref' output that lists the numbers of matches for each reference sequence in each scanned file>
...
##
##  Copyright 2020, Andrew Fire and Stanford University
...
while ai1<len(argv):
    ...
    if a11.startswith('exclude'):
        ...
        if a22.lower().startswith('f'):
            ...
            FileByFileMatches1 = True
    elif a11.startswith('runstosamples') or a11.startswith('samplestoruns') :
        for L0 in open(a22,mode='rU'):
            L1 = L0.strip().split('\t')
            Sample1 = L1[0]
            AllSamples1.append(Sample1)
            for run1 in L1[1].split(','):
                RunsToSamples1[run1] = len(AllSamples1)-1
    ...
    elif a11.startswith('matchtable'):
        if a22.lower().startswith('f'):
            RefByRef1 = False
        else:
            RefByRef1 = True
...
if RunsToSamples1:
    for df0 in DataFile0:
        ...
        if Run1 in RunsToSamples1:
            Sample1 = RunsToSamples1[Run1]
            if Sample1 % Multi2==Multi1:
...
for iD0,nD0 in enumerate(DataFile1):
    ...
    if RefByRef1:
        ...
        if Run1 in RunsToSamples1:
            ...
            continue
        if Sample1 in SamplesToColumns1:
            Column1 = SamplesToColumns1[Sample1]
        else:
            Column1 = len(SamplesToColumns1)
            SampleList1.append(AllSamples1[Sample1])
            SamplesToColumns1[Sample1] = Column1
...
if RefByRef1:
    ...
    F3.write('SeqID\tSequence\t'+'\t'.join(SampleList1)+'\r')
    for i,(n,s) in enumerate(izip(NameList1,SeqList1)):
        F3.write(n+'\t'+s)
        for j in range(len(MatchList1)):
